...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
	...
	private void draw(GL2 gl2) {
		...
		gl2.glBegin( gl2.GL_TRIANGLES );//red
		gl2.glColor3f( 1.0f, 1.0f, 1.0f ); 
		/*
		gl2.glVertex3f( 2.0f, 0.f, 0.0f ); 		
		gl2.glVertex3f( -2.0f, 0.f, 0.0f ); 				
		gl2.glVertex3f( 0.0f, 2.f, 0.0f ); */
		...
		float r=2;
			...
			for(int i=0;i<sides;i++){
				...
				gl2.glVertex3f((float)(r*Math.cos(angle)),(float)(r*Math.sin(angle)),0.0f);	
				 angle=(i+1)*2*Math.PI/sides;
				gl2.glVertex3f((float)(r*Math.cos(angle)),(float)(r*Math.sin(angle)),0.0f);	
	...
	private void draw2(GL2 gl2)
	{
		gl2.glRotatef(rotateZ,0,0,1);   // Apply rotations to complete object.
		gl2.glRotatef(rotateY,0,1,0);
		gl2.glRotatef(rotateX,1,0,0);

		gl2.glBegin( gl2.GL_LINES );//red
	    ...
	    //gl2.GLfloat lineWidthRange[2] = {0.0f, 0.0f};
	   // gl2.glGetFloatv(gl2.GL_ALIASED_LINE_WIDTH_RANGE, lineWidthRange);		
	    float height=4;
		for(int j=0; j<6;j++) {
			r+=0.5;
			for(int i=0;i<sides;i++){
				...
				double angle=i*2*Math.PI/sides;
				gl2.glVertex3f((float)(r*Math.cos(angle)),(float)(r*Math.sin(angle)),height);	
				 angle=(i+1)*2*Math.PI/sides;
				gl2.glVertex3f((float)(r*Math.cos(angle)),(float)(r*Math.sin(angle)),height);
				height-=0.1f;
			}
